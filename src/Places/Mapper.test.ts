...
describe('PlacesMapper', () => {
	...
	describe('#mapPlaceApiResponseToPlaces', () => {
		...
		it('should correctly map api response to array of Places', (done) => {
			const apiResponse: ApiResponse = new ApiResponse('200', 200, '', TestData.placesResponse);

			/* tslint:disable */
			const expectedMappedPlaces: Place[] = [{
				guid: 'poi:345430',
				level: 'poi',
				rating: 10.004,
				location: {
			...
			chai.expect(Mapper.mapPlaceApiResponseToPlaces(apiResponse)).to.deep.equal(expectedMappedPlaces);
			done();
		});
	});

	describe('#mapPlaceDetailedApiResponseToPlace', () => {
		it('should correctly map api response to single Place with PlaceDetail', (done) => {
			const apiResponse: ApiResponse = new ApiResponse('200', 200, '', TestData.placeDetailedWithNoMediaResponse);

			/* tslint:disable */
			const expectedMappedPlaceDetailed: Place = {
				guid: 'poi:530',
				level: 'poi',
			...
			chai.expect(Mapper.mapPlaceDetailedApiResponseToPlace(apiResponse, '400x400'))
				.to.deep.equal(expectedMappedPlaceDetailed);
	...
	describe('#mapMainMediaToMedia', () => {
		it('should correctly map media from place detailed api response', (done) => {
			const mainMedia: Media.MainMedia = camelizeKeys(TestData.placeDetailMedia) as Media.MainMedia;
			const expectedMappedMedia = {
				square: {
					original: {
						size: 9511064,
						width: 4114,
			...
			chai.expect(Mapper.mapMainMediaToMedia(mainMedia, '400x400')).to.deep.equal(expectedMappedMedia);
			done();
		});
	});
});
